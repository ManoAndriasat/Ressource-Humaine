...
namespace RessourceHumaine
{
    public class CandidatModel
    {
        ...
        public static CandidatModel SelectCandidatByID(string ID_Candidat)
        {
            ...
            public static List<CandidatModel> getAll() {
        List<CandidatModel> allCandidat = new List<CandidatModel>();

        using (NpgsqlConnection conn = new Connection().GetConnection()) {
            if (conn != null) {
                using (NpgsqlCommand cmd = new NpgsqlCommand("SELECT * FROM Candidat", conn)) {
                    using (NpgsqlDataReader reader = cmd.ExecuteReader()) {
                        while (reader.Read()) {
        ...
        // insertion candidat embauche
        public static void InsertCandidatEmbauche(string idCondidat, string dateNaissance, string dateEmbauche, string numCIN, string contact, string direction, string fonction) {
            using (NpgsqlConnection conn = new Connection().GetConnection())
            {
                if (conn != null)
                {
                    ...
                    {
                        cmd.Connection = conn;
                        cmd.CommandText = "INSERT INTO Candidat_embauche VALUES ('" + GetMatricule() + "', '" + idCondidat + "', '" + dateNaissance + "', '" + dateEmbauche + "', '" + numCIN + "', '" + contact + "', '" + direction + "', '" + fonction + "')";
        ...
        public static int GetMatricule()
        {
            ...
            using (NpgsqlConnection conn = connection.GetConnection())
            {
                if (conn != null)
                {
                    using (NpgsqlCommand cmd = new NpgsqlCommand("SELECT nextval('Matricule')", conn))
                    {
                        object result = cmd.ExecuteScalar();
                        if (result != null)
                        {
